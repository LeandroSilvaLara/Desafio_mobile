...
import com.leandro.desafio_mobile.databinding.ActivityMovieDetailsBinding
import com.leandro.desafio_mobile.ui.viewmodels.MovieDetailsViewModel
...
class MovieDetailsActivity : AppCompatActivity() {
    ...
    private fun changeButtonFavoriteStyle(status: Boolean = false) {
        binding.buttonFavorite.backgroundTintList = ContextCompat.getColorStateList(
            this,
            if (status) R.color.yellow else R.color.background_theme
        )
        binding.buttonFavorite.text =
            getString(if (status) R.string.button_remove_favorite else R.string.button_favorite)
        binding.buttonFavorite.setTextColor(
            ContextCompat.getColor(
                this,
                if (status) R.color.black else R.color.yellow
            )
        )
        binding.buttonFavorite.icon = AppCompatResources.getDrawable(
            this,
            if (status) R.drawable.ic_favorite_color else R.drawable.ic_favorite
        )
        binding.buttonFavorite.iconTint =
            ContextCompat.getColorStateList(this, if (status) R.color.black else R.color.yellow)
        if (!status) {
            binding.buttonFavorite.strokeWidth = 1
            binding.buttonFavorite.strokeColor =
    ...
    private fun setupHooks() {
        ...
        /**
         * Fazer alterações no andamento do projeto
         */

        binding.buttonListComprar.setOnClickListener {
            if (isMovieAlreadySaved) {
                viewModel.deleteMovie(idMovieEntity!!)
            } else {
                viewModel.saveMovie(movieResponse.toMovieEntity())
